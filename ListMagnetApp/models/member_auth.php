       ...
       function getAuthUsers()  
		...
		function newMemberInsert($data){
     	...
     	function getFacebookMember($id) {  
               $query = $this->db->get_where('member_auth',array('facebook_id' => $id)); 
		...
		function getTwitterMember($id) {  
               //Query the data table for every record and row  
               $query = $this->db->get_where('member_auth',array('twitter_id' => $id));  
     			//var_dump($query);
               if ($query->num_rows() <= 0)  
               {  
                   return NULL; 
		...
		function getGoogleMember($id) {
               ...
               $query = $this->db->get_where('member_auth',array('google_id' => $id));  

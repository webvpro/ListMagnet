<?php
	...
	class tweet {
		...
		function __construct()
		{
			...
			$this->user_id=$this->logged_in_user();
			$this->userObj= $this->get_user_info();
			$this->isLogged= $this->logged_in();
		...
		private function _manage_session(){
			...
			if ( $user === FALSE && $this->isLogged !== FALSE )
			{
				
				$userObj = $this->call('get', 'account/verify_credentials');
				//echo "Tweet is logged:";
				//var_dump($this->isLogged);
      			//$this->set_tokens($this->get_tokens);
		        $user=  array(
		           		'user_name'    => $userObj->screen_name,
				...
				$mem=$this->_obj->Member_auth->getTwitterMember($userObj->id_str);
				//var_dump($mem);
				if($mem > 0){
			...
			elseif ( $user !== FALSE && $this->isLogged === FALSE )
			{
